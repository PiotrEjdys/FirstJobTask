...
export class BooksListComponent implements OnInit {
  ...
  @Output() detailDisplayEmitter = new EventEmitter<{
    display: boolean;
    index: number;
  }>();
  books: Book[];
  ...
  showDetails(index: number) {
    ...
    this.detailDisplayEmitter.emit({
      display: this.detailDisplay,
      index: index,
    });
  }
  deleteBook(index: number) {
    if (
      confirm(
        'Are u sure u want to delete book ' +
          this.bookService.getOneBook(index).title
      )
    ) {
